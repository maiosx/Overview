...
Item {
  ...
  readonly property string thumbCache: Format.videoThumbPath(path, homePrefix)
    ...
    Column {
      ...
      Item {
        ...
        Image {
          ...
          source: {
            if (root.isImage) return Format.fileUrl(root.path)
            if (root.isVideo && root.thumbCache.length)
          ...
          onStatusChanged: {
            if (root.isVideo && status === Image.Error)
              thumbRetry.restart()
          }
        }
        ...
        Timer {
          ...
          onTriggered: {
            if (!root.isVideo || thumb.status === Image.Ready) { tries = 0; return }
        ...
        Text {
          ...
          visible: !root.isImage && !(root.isVideo && thumb.status === Image.Ready)
